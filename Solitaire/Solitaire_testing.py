            ...
            if bottomright_Y > y > topleft_Y:
                if clickednum == 0:
                    if layer[-1].get_exposed() == True:
                        clickedlayer1 = layer
                        # Keeps track of whether card1 is from shuffled or layer
                        layer_or_shuf1 = "layer"
                        clickednum = 1
                        card1 = layer[-1]
                    else:
...
def put_in_series():
    ...
    global layer_or_shuf2, card1, card2, shuffled, shufflenum
    ...
    if clickedlayer2 == " ":
        ...
        if layer_or_shuf1 == "layer" and layer_or_shuf2 == "layer":
            card1colour = card1.get_colour()
            card2colour = card2.get_colour()
        
            card1val = card1.get_val()
            card2val = card2.get_val()
            # fixes bug where game crashes if two of the same value card are selected
            if card1.get_val() != card2.get_val():
        ...
        # fixes bug where game crashes if two of the same value card are selected
        elif layer_or_shuf1 == "shuffled" and layer_or_shuf2 == "layer":
            card1 = shuffled[shufflenum]
            print card1
            print card2
        ...
        for selectedcard in range(len(cards_shown)):
            ...
            if card_left < x < card_right:
                if card_top < y < card_bot:
                    if clickednum == 0:
                        layer_or_shuf1 = "shuffled"
                        clickedlayer1 = cards_shown
                        clickednum = 1
                        shufflenum = selectedcard
# Event Handlers
